...
if (isset($_GET['action'])){
	$dat=Array();
if ($_GET['target']==='prod'){
		$dat=unserialize(file_get_contents('produits.dat'));
		$record='produits.dat';
	...
	if ($_GET['action']==='up'&&is_numeric($_GET['index'])) {
		...
		if ($_GET['index']==count ($dat)-1){
			...
			$after=array_slice($dat, $_GET['index']+1, -1);
		...
		$after=array_slice($dat, $_GET['index']+2, -1);
...
echo '<h1>Modification de l\'ordre d\'affichage des produits</h1><h2>L\'ordre d\'affichage des catégories primera toujours sur l\'ordre individuel des produits</h2><ol>';
 ...
 <li>Produit : <em><?php echo htmlspecialchars($prod['nom']);?></em>
   (<?php echo htmlspecialchars($prod['categorie']);?>)
   
    <?php echo htmlspecialchars($prod['description']);?>
      <?php echo htmlspecialchars($prod['prix']);?>
      				<?php
				
				if ($j!==0) {
					echo '<a href="?action=up&target=prod&index='.$j.'">Monter</a> ';
				}
				if ($j!=count($prods)-1) {
					echo '<a href="?action=down&target=prod&index='.$j.'">Descendre</a>';
				}
